...
const UserInfo = ({ token, setToken }: LoginProps) => {
  ...
  useEffect(() => {
    import('persona').then((Persona) => {
      const personaClient = new Persona.Client({
        templateId: "itmpl_EsMwKdukXWENFuBxoeswnXgkULxD",
        environmentId: "env_nk87LS5D56CTE617UUfGrVtbQ1pZ",
  ...
  return (
    <Card>
      ...
      <FormButton onClick={showWallet} disabled={false}>
        Show Wallet
      </FormButton>
      <FormButton onClick={startKYC} disabled={false}>
        Start KYC
      </FormButton>
    </Card>
  );
